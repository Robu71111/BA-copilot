"""
Process Flow Diagram Route — async httpx
        ...
        for model in APIConfig.FREE_MODELS:
            try:
                ...
                if APIConfig.should_skip(body):
                    print(f"[flow] Skip signal on {model}"); await asyncio.sleep(0.5); continue

                data = response.json()
                if "error" in data:
                    last_error = str(data["error"])[:150]
                    print(f"[flow] Error on {model}: {last_error}"); await asyncio.sleep(0.5); continue
                ...
                print(f"[flow] Exception on {model}: {last_error}"); await asyncio.sleep(0.3); continue
